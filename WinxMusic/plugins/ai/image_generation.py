...
from WinxMusic.helpers.misc import getText, ImageModels
...
async def generate(_, message: Message):
    ...
    btns = [[btns[0], btns[1]], [btns[2], btns[3]], [btns[4], btns[5]], [btns[6], btns[7]], [btns[8], btns[9]], [btns[10], btns[11]], [btns[12], btns[13]]]
    await message.reply_animation(config.GLOBAL_IMG_URL,
                                  caption=f"➜ Escolha um modelo", reply_markup=InlineKeyboardMarkup(btns))
...
async def draw(_, query):
    ...
    if promptdata is None:
        return await query.edit_message_text("algo deu errado, tente novamente mais tarde")
    ...
    try:
        ...
        for i in img_url:
            images.append(InputMediaDocument(i, caption=f"➜ prompt: {promptdata['prompt']}\n\npoe: @{app.me.username}"))
