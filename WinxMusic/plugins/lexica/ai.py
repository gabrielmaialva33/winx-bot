...
from WinxMusic.helpers.lexica_miscs import get_text, get_media


@app.on_message(filters.command([i for i in dir(languageModels) if not i.startswith("__")]))
...
async def ask_about_image(_: Client, m: t.Message, media_files: list, prompt: str):
    ...
    for media in media_files:
        ...
        images.append(image)
    output = await lexica_gemini_vision(prompt if prompt else "o que é isso?", "geminiVision", images)
